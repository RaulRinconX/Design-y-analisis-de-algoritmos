...
public class Main3 {
    private static PriorityQueue<Map.Entry<Integer, Set<Integer>>> politicians = new PriorityQueue<>(
            (a, b) -> Integer.compare(b.getValue().size(), a.getValue().size())
    );
    private static HashMap<Integer, Set<Integer>> politiciansMap = new HashMap<>();
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int t = 0; t < T; t++) {
            ...
            for(int m = 1; m <= M; m++) {
                politiciansMap.put(m, new HashSet<>());
            }
            for(int n = 1; n <= N; n++) {
                citizens.add(n);
                ...
                for(String s : line) {
                    ...
                    politiciansMap.get(politician).add(n);
                }
            }
            politicians.addAll(politiciansMap.entrySet());
            greedySetCover();
            politicians.clear();
            politiciansMap.clear();
            citizens.clear();
        }
        br.close();
    }
    ...
    private static void greedySetCover() {
        ...
        while (!citizens.isEmpty() && !politicians.isEmpty()) {
            Map.Entry<Integer, Set<Integer>> entry = politicians.poll();
            ...
            //  requiredPoliticians.add(entry.getKey());
                ...
                for (Map.Entry<Integer, Set<Integer>> e : politicians) {
                    e.getValue().removeAll(intersect);
                }
                politicians.removeIf(e -> e.getValue().isEmpty());
                requiredPoliticians.add(entry.getKey());
            }
        }
